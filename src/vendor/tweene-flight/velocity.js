 ...
 */
    function compoundMapping(name, value) {
      ...
      prefix = nameParts[0]
      suffix = nameParts.length > 1
        ? nameParts[1].substr(0, 1).toUpperCase() + nameParts[1].substr(1)
        : ''

      dirs = name == 'borderRadius' ? radiusDirections : compoundDirections

      for (i = 0; i < 4; i++) {
        values[prefix + dirs[i] + suffix] = easing
          ? [parts[i], easing]
          : parts[i]
      }
      return values
    ...
    function transformMapping(name, value) {
      ...
      if (name == 'rotate3d') {
        ...
        switch (parts.length) {
          ...
          case 1:
            parts = baseName == 'rotate' || baseName == 'rotation'
              ? [null, null, parts[0]]
 ...
 */
      ...
      var _drivers = {
        ...
        _getInstance = function(type, driverName) {
          ...
          driverName = (driverName
            ? driverName
            : _self.defaultDriver).toLowerCase()
     ...
     */
      this.line = function(target, options, driver) {
        ...
        var lineTarget = (isObject(target) && !seemsPlainObject(target)) ||
          isString(target)
          ? target
          : null
        ...
        driver = driver !== void 0
          ? driver
          : 'driver' in options ? options.driver : null
 ...
 */
     ...
     */
      this.duration = function(value) {
        ...
        return (
          Math.round(
            convertTime(this._duration, this._coreTimeUnit, this._timeUnit) *
              1000
          ) / 1000
        )
      }
     ...
     */
      this.totalDuration = function() {
        ...
        return (
          Math.round(
            convertTime(
              this._getTotalDuration() * this._speed,
              this._coreTimeUnit,
              this._timeUnit
            ) * 1000
          ) / 1000
          ...
          if (callback === null) {
            ...
            this._handlers[name] = {
              ...
              params: arguments.length > 2
                ...
                : [],
              scope: arguments.length > 3 && arguments[3] !== null
                ? arguments[3]
                : this,
            }
          }
     ...
     */
      this.getRealSpeed = function() {
        return this._parent
          ? this._parent.getRealSpeed() * this._speed
          : this._speed
      }
     ...
     */
      this._getTotalDuration = function() {
        ...
        return (
          (this._duration + (this._loopsDelay + this._duration) * this._loops) /
          this._speed
        )
      }
     ...
     */
      this._applyArguments = function(args) {
        ...
        for (name in args) {
          ...
          if (
            this.type != 'timeline' &&
            inArray(['from', 'to', 'then', 'immediateStart'], name) != -1
          ) {
     ...
     */
      this._hasHandlers = function(name) {
        return (
          (name in this._handlers && this._handlers[name] !== null) ||
          this._coreHandlers[name].length
        )
      }
     ...
     */
      this._parseOptions = function(options, remove) {
        var opts = this.type == 'tween'
          ? extendObject({}, optionsMap, tweenOptionsMap)
          : optionsMap,
          values = {},
          name,
          realName,
 ...
 */
          ...
          if (args.length) {
            ...
            if (args.length) {
              ...
              if (args.length) {
                ...
                if (
                  !('duration' in values) &&
                  (isString(args[0]) || isNumber(args[0]))
                ) {
                  values.duration = args.shift()
                }
        ...
        if (!this._ready) {
          ...
          this._data = {
            delay: convertTime(
              this._delay,
              this._coreTimeUnit,
              this._driverTimeUnit
            ),
            loopsDelay: convertTime(
              this._loopsDelay,
              this._coreTimeUnit,
              this._driverTimeUnit
            ),
          ...
          this._data.tween = this._prepareProperties(
            this._from,
            this._to,
            this._then
          )
     ...
     */
      this._prepareSingle = function(tween, block, type) {
        if (block) {
          ...
          var hasField =
            '_has' + type.substr(0, 1).toUpperCase() + type.substr(1)
     ...
     */
      this._splitEasing = function(tween) {
        var map = {}, tweens = [], name, easing, easingName, entry
        ...
        for (name in tween) {
          ...
          easingName = isString(easing)
            ? easing
            : easing.join('_').replace(/\./g, 'p')
        ...
        for (var name in values) {
          if (values.hasOwnProperty(name)) {
            ...
            if (inArray(compoundNames, newName) !== -1) {
              ...
              this._allowTransform &&
              inArray(compoundTransforms, newName) !== -1
            ) {
            ...
            for (name in subValues) {
              ...
              newName = name in this._propertyMap
 ...
 */
     ...
     */
      this.add = function(child, startPosition) {
        ...
        if (isString(child)) {
          ...
          if (
            isFunction(child) ||
            (isNumber(child) && isFunction(startPosition))
          ) {
            var dir = 0, i = 0
            ...
            var params = arguments.length > i
              ? isArray(arguments[i]) ? arguments[i] : [arguments[i]]
              : []
            i++
     ...
     * @returns {number}
     */
      this._getStartPosition = function(
        currentDuration,
        currentCursor,
        startPosition
      ) {
 ...
 */
     ...
     */
      this.resume = function() {
        ...
        if (
          this._paused &&
          ((this._fwd && this._playAllowed) ||
            (!this._fwd && this._reverseAllowed))
        ) {
          this._paused = false
          this.prepare()
          ...
          if (this.type != 'timeline') {
            var duration = this._localFwd
              ? this._duration - this._position
              : this._position
            this._data.duration = convertTime(
              duration,
              this._coreTimeUnit,
              this._driverTimeUnit
            )
          ...
          if (this._running) {
            ...
            if (
              this.type != 'timeline' &&
              ((this._position === 0 && this._localFwd) ||
     ...
     */
      this._back = function() {
        ...
        if (this._running) {
          this._delayDummy = null
          var dir = this._localFwd
            ? this._hasPre && this._offset !== 0 ? 'pre' : 'begin'
            : 'end'
          this._backTween(dir)
        }
        return this
 ...
 */
    function reverseBezier(value) {
      return [1 - value[2], 1 - value[3], 1 - value[0], 1 - value[1]]
    }
 ...
 */
    function getNewBezier(oldBezier, time) {
      ...
      var cacheName =
        oldBezier.join('_').replace(/\./g, 'p') + '_' + time.toFixed(3)
      ...
      var p0x = 0,
        p0y = 0,
        p1x = u,
        p1y = (oldBezierFunc(uu) - startY) * sign / yInterval,
        p2x = v,
        p2y = (oldBezierFunc(vv) - startY) * sign / yInterval,
        p3x = 1,
        p3y = 1,
        compU = 1 - u,
        compV = 1 - v,
      ...
      this._getTweenValues = function(tween, field, isSet) {
        ...
        for (name in tween) {
          ...
          if (entry[field] !== null) {
            ...
            value = isNumeric(entry[field])
              ? Number(entry[field])
              : entry[field]
            value = isSet || !this._allowMultipleEasing || !easing
              ? value
              : [value, this._getRealEasing(easing)]
            values[name] = value
          }
        }
        ...
        for ((i = 0), (end = this._getTargetLength()); i < end; i++) {
          ...
          for (name in names) {
            ...
            if (field.mask > 0) {
              ...
              tweenValue = field.begin != hidden
                ? field.begin
                : field.end != hidden ? field.end : false
              if (tweenValue !== false) {
 ...
 */
      ...
      this.prepare = function() {
        ...
        for (var len = this._index.length - 1; i < len; i++) {
          ...
          if (!keyframe.fTrigger) {
            j = i + 1
            while (
              j < this._index.length - 2 &&
              !this._keyframes[this._index[j]].fTrigger
            ) {
              j++
            }
     ...
     */
      this._addDummy = function(begin, end) {
        ...
        dummy
          .parent(this)
          .duration(end - begin)
          .setCoreHandler('reverse', 'timeline', this._childCallback, this, [
            'b',
            begin,
            dummy.id(),
            res[0],
          ])
          .setCoreHandler('end', 'timeline', this._childCallback, this, [
            'f',
            end,
            dummy.id(),
            res[1],
     ...
     */
      this._mergeElement = function(child, begin, end, tweenable) {
        ...
        if (tweenable) {
          ...
          child.setCoreHandler(
            'reverse',
            'timeline',
            this._childCallback,
            this,
            ['b', begin, child.id(), res[0]]
          )
          if (end != Infinity) {
            child.setCoreHandler('end', 'timeline', this._childCallback, this, [
              'f',
              end,
              child.id(),
              res[1],
     ...
     */
      ...
      this._processKeyframe = function(time, direction, currentIndex) {
        ...
        var cDirection = direction + 'c',
          cList = this._keyframes[time][cDirection],
          tList = this._keyframes[time][direction],
          i,
          end,
          offset,
          item,
        ...
        if (!paused) {
          ...
          if (
            (direction == 'b' && time === 0) ||
            (direction == 'f' && time == this._index[this._index.length - 1])
          ) {
        ...
        if (!paused) {
          if (runningCount) {
            ...
            if (direction && this._position === 0) {
              ...
              args = [
                'b',
                this._index.length ? this._index[this._index.length - 1] : 0,
                -1,
                true,
              ]
            }
 ...
 */
    ...
    Tw.registerDriver('Dummy', 'tween', function() {
      ...
      this.setCoreHandler('end', 'resetPos', this._resetPosition, this)
      ...
      this.resume = function() {
        if (this._paused) {
          ...
          var duration =
            (this._localFwd
              ? this._duration - this._position
              : this._position) / this.getRealSpeed()
          if (!duration) {
 ...
 */
     ...
     */
      this._getVelocityValues = function(tween, callback) {
        ...
        var name,
          ...
          beginValue,
          endValue,
          entry = calls[calls.length - 1][0][0]
        for (name in entry) {
          if (isObject(entry[name]) && 'startValue' in entry[name]) {
            ...
            if (!(name in tween)) {
              tween[name] = {
                ...
                then: null,
                easing: root in tween ? tween[root].easing : null,
              }
            }
     ...
     */
      this._fetchThen = function(tween, name, root, begin, end) {
        ...
        if (root != name) {
          tween[root].begin = tween[root].end = tween[root].then = tween[
            root
          ].pre = null
        }
      }
     ...
     */
      this._fetchEnd = function(tween, name, root, begin, end) {
        ...
        if (root != name) {
          tween[root].begin = tween[root].end = tween[root].pre = tween[
            root
          ].then = null
        }
      }
     ...
     */
      this._playTween = function() {
        ...
        var self = this,
          ...
          target,
          onBegin = function() {
            self._runHandlers('_begin')
          },
          onComplete = function() {
            self._runHandlers('_end')
          },
          onProgress = function() {
            self._runHandlers('progress')
          }
        for ((i = 0), (end = this._target.length); i < end; i++) {
          tween = data.tween[i]
 ...
 */
  ...
  module.exports = function(env) {
    if (window && !window.jQuery) window.jQuery = window.$ = env.jQuery
    var mod = require('velocity-animate')
    return func(env)
  }
